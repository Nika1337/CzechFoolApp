...
class OfflinePlayersRepository(
    ...
    override suspend fun insertAll(vararg players: Player) = playerDao.insertAll(*(players.map { it.toPlayerEntity() }.toTypedArray()))

    override suspend fun delete(player: Player) {
        playerDao.delete(player.toPlayerEntity())
    }

    override suspend fun update(player: Player) = playerDao.update(player.toPlayerEntity())
